...
const triplestoreState = {
	finalAnswerContent:
		'{"district":"Connewitz","amount":"120 Liter","year":"2025"}',
	clarificationResponses: [] as Array<
		Array<{ uri: unknown; content: string }>
	>,
	...
	HttpClientResponse: {
		filterStatusOk: (response: { status: number }) =>
			response.status >= 200 && response.status < 300
				...
				: Effect.fail(new Error("Not OK")),
		schemaBodyJson: (_schema: unknown) => (response: { body: unknown }) =>
			...
			queryClarifications: (_graphUri: string) =>
				Effect.succeed(
					triplestoreState.clarificationResponses[
						triplestoreState.clarificationCallCount++
					] ?? []
				),
...
beforeEach(() => {
	...
	llmState.clarificationText =
		"In welchem Stadtteil oder an welcher Adresse suchst du einen Baum?";
	llmState.rewritePrefix = "rewritten:";
	triplestoreState.finalAnswerContent =
		'{"district":"Connewitz","amount":"120 Liter","year":"2025"}';
	triplestoreState.clarificationResponses = [];
