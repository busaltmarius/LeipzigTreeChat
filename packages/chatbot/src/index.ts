...
// Build the graph
export const ChatBotGraph = (printMessage: (message: BaseMessage) => Promise<void>, getUserInput: () => Promise<string>) => {
    const { UserInputNode, ResponseNode, RouterNode } = Nodes(printMessage, START, END, CHATBOT_RESPONSE_NODE_ID, ROUTER_NODE_ID, USER_INPUT_NODE_ID);
