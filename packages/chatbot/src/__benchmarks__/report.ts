...
import type { BenchmarkCaseResult, BenchmarkFixture, BenchmarkRunResult, BenchmarkStatus } from "./types.js";
...
const filterByStatus = (results: BenchmarkCaseResult[], status: BenchmarkStatus): BenchmarkCaseResult[] =>
  ...
  return [...results]
    .sort((left, right) => {
      if (left.evaluation.status !== right.evaluation.status) {
        ...
        return order.indexOf(left.evaluation.status) - order.indexOf(right.evaluation.status);
      }
...
export const renderBenchmarkMarkdown = (runResult: BenchmarkRunResult): string => {
  ...
  const gracefulFailureCases = runResult.cases.filter((result) => hasGracefulFailureExpectations(result.fixture));
  const clarificationSuccessRate =
    clarificationCases.length === 0
      ? 1
      : clarificationCases.filter(
          (result) => result.evaluation.axes.find((axis) => axis.axis === "flow_correctness")?.passed
        ).length / clarificationCases.length;
  const gracefulFailureSuccessRate =
    gracefulFailureCases.length === 0
      ? 1
      : gracefulFailureCases.filter(
          (result) => result.evaluation.axes.find((axis) => axis.axis === "graceful_failure")?.passed
...
export const writeBenchmarkArtifacts = async (
  ...
  const jsonPath = resolve(resolvedOutputDir, `demo-benchmark-${safeTimestamp}.json`);
  const markdownPath = resolve(resolvedOutputDir, `demo-benchmark-${safeTimestamp}.md`);
