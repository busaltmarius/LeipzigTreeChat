...
const AXES: BenchmarkAxis[] = ["task_completion", "grounding", "flow_correctness", "graceful_failure", "presentation"];
...
const selectSignalTargetText = (
  ...
  switch (target ?? "assistant_last") {
    ...
    case "transcript":
      return caseResult.transcript.map((entry) => `${entry.role}: ${entry.content}`).join("\n");
    case "metadata":
...
const buildFlowChecks = (fixture: BenchmarkFixture, caseResult: BenchmarkCaseResult): BenchmarkCheckResult[] => {
  ...
  if (fixture.expectedFlow.requiredMetadataStatuses?.length) {
    checks.push({
      ...
      critical: true,
      passed: includesStatusesInOrder(observedStatuses, fixture.expectedFlow.requiredMetadataStatuses),
  ...
  if (fixture.expectedFlow.clarification === "required") {
    checks.push({
      ...
      details: clarificationObserved ? "Clarification metadata observed." : "No clarification metadata observed.",
    });
  }
...
const buildPresentationChecks = (caseResult: BenchmarkCaseResult): BenchmarkCheckResult[] => {
  const lastAssistantMessage =
    caseResult.transcript
      ...
      .at(-1) ?? "";
  const looksGerman =
    /\b(der|die|das|und|ich|du|nicht|bitte|kann|gibt|wurde|wurden|sind|ist|welche|welcher|in|gegossen|fragen|beantworten|Leipzig|Baum|Bäume|Wasser|Danke)\b|[äöüß]/i.test(
      lastAssistantMessage
    );
  const looksStructuredButUnreadable =
    lastAssistantMessage.trim().startsWith("{") && lastAssistantMessage.trim().endsWith("}");
...
const summarizeAxis = (checks: BenchmarkCheckResult[], defaultSummary: string): BenchmarkAxisResult => {
  ...
  const summary = passed ? defaultSummary : failedChecks.map((check) => check.label).join("; ");
...
export const scoreBenchmarkCase = (fixture: BenchmarkFixture, caseResult: BenchmarkCaseResult): BenchmarkEvaluation => {
  const axisChecks = new Map<BenchmarkAxis, BenchmarkCheckResult[]>(AXES.map((axis) => [axis, []]));
  ...
  if ((axisChecks.get("task_completion")?.length ?? 0) === 0) {
    axisChecks.get("task_completion")?.push({
      ...
      critical: true,
      passed: caseResult.transcript.some((entry) => entry.role === "assistant" && entry.turnIndex !== null),
