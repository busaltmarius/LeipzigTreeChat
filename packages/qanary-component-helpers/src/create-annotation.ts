...
export const createAnnotationInKnowledgeGraph = async ({
  ...
  // if the annotationType start with "http" or "urn:" then add "<" and ">" else use the annotationType directly
  annotationType =
...
WHERE {
  ...
  try {
    ...
    console.error("Error creating annotation in Qanary triplestore");
    console.error(error);
  }
};
