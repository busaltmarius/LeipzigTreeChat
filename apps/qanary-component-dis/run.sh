...
bun run build --workspace=api && \
  ...
  bun run build --workspace=qanary-component-dis
...
# 2) run all tests
echo "Running tests"
cd "$ROOT_DIR/apps/qanary-component-dis" && bun test

# 3) start the application (Bun runtime)
cd "$ROOT_DIR/apps/qanary-component-dis"
env "SPRING_BOOT_ADMIN_CLIENT_INSTANCE_SERVICE-BASE-URL=$SPRING_BOOT_ADMIN_URL" \
  "SPRING_BOOT_ADMIN_URL=$SPRING_BOOT_ADMIN_URL" \
  "QANARY_COMPONENT_PACKAGE_JSON=$ROOT_DIR/apps/qanary-component-dis/package.json" \
