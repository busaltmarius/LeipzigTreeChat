...
import { createAnnotationInKnowledgeGraph, selectSparql, getEndpoint, getOutGraph } from "@leipzigtreechat/qanary-component-helpers";
...
export const mapDataToTemplate = async(relation: string, entityId: string): Promise<string> => {
  if(relation == "Wie viel wurde im Stadteil Kleinzschocher gegossen?") {
    ...
    `
  } else if(relation == "Welche Wasserentnahme Stellen gibt es in der Nähe der Adresse Karl-Liebknecht-Str. 132, 04277 Leipzig?") {
    return ""
  } else if(relation == "Welchen Baum kann ich in der Nähe der Adresse Karl-Liebknecht-Str. 132, 04277 Leipzig heute gießen?") {
    return `
