...
const ELIGIBLE_EXTENSIONS = new Set([".cjs", ".cts", ".js", ".jsx", ".mjs", ".mts", ".ts", ".tsx"]);
...
const packageSlug = packageName.startsWith("@") ? packageName.split("/").at(-1) : packageName;
...
const sourceFiles = existsSync(sourceRoot) ? await collectSourceFiles(sourceRoot) : [];
const manifestFiles = [];
const symbols = [];
...
function shouldIncludeFile(sourceRelativePath, currentPackageName) {
  ...
  if (currentPackageName === "@leipzigtreechat/qanary-api" && GENERATED_QANARY_API_DIRS.has(segments[0] ?? "")) {
    return false;
  }
    ...
    return [
      createSymbolRecord({
        ...
        params: statement.parameters.map((parameter) => ({
          description:
            docComment?.params.find((docParam) => docParam.name === parameter.name.getText(sourceFile))?.description ??
            "",
  ...
  if (ts.isInterfaceDeclaration(statement)) {
    ...
    return [
      createSymbolRecord({
        ...
        properties: collectInterfaceProperties(statement, sourceFile, context.sourceText),
    ...
    return [
      createSymbolRecord({
        ...
        properties: statement.members.map((member) => ({
          description: getParsedDocComment(member, sourceFile, context.sourceText)?.summary ?? "",
  ...
  if (ts.isVariableStatement(statement)) {
    ...
    return statement.declarationList.declarations.map((declaration) => {
      ...
      if (functionInitializer) {
        return createSymbolRecord({
          ...
          params: functionInitializer.parameters.map((parameter) => ({
            description:
              docComment?.params.find((docParam) => docParam.name === parameter.name.getText(sourceFile))
                ?.description ?? "",
            name: parameter.name.getText(sourceFile),
            type: parameter.type?.getText(sourceFile) ?? "unknown",
          })),
      ...
      return createSymbolRecord({
        ...
        node: declaration,
        docComment,
        kind: (statement.declarationList.flags & ts.NodeFlags.Const) !== 0 ? "constant" : "member",
        name: declaration.name.getText(sourceFile),
        signature: renderVariableSignature(
          declaration,
          sourceFile,
          (statement.declarationList.flags & ts.NodeFlags.Const) !== 0 ? "const" : "let"
        ),
...
function createSymbolRecord({
  ...
  const lineAndCharacter = context.sourceFile.getLineAndCharacterOfPosition(node.getStart(context.sourceFile));
...
function collectClassProperties(node, sourceFile, sourceText) {
  return node.members
    ...
    .map((member) => ({
      description: getParsedDocComment(member, sourceFile, sourceText)?.summary ?? "",
      name: member.name.getText(sourceFile),
...
function renderFunctionSignature(node, sourceFile) {
  const parameters = node.parameters.map((parameter) => renderParameter(parameter, sourceFile)).join(", ");
...
function renderConstructorSignature(node, sourceFile) {
  return `constructor(${node.parameters.map((parameter) => renderParameter(parameter, sourceFile)).join(", ")})`;
}
...
function getParsedDocComment(node, sourceFile, sourceText) {
  ...
  return {
    ...
    params: docComment.params.blocks.map((paramBlock) => ({
      description: normalizeMarkdown(renderDocNodes(paramBlock.content.getChildNodes())),
